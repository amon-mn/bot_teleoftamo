...
- `src/navigation.py` — `open_consulta_laudos(driver)`: navega direto para a URL de Consulta de Laudos (`CONSULTA_LAUDOS_URL` em `locators.py`), já autenticado — decisão tomada durante a tarefa 4.1: o clique no card `a.tiles-toyo` lançava `ElementNotInteractableException` no Selenium, e a navegação direta por URL é equivalente em efeito (chega na mesma tela) e mais robusta; `apply_location_filter(driver, estado, cidade)`: seleciona Estado e Cidade nos `<select>`, comparando o nome da cidade sem diferenciar maiúsculas/minúsculas (as opções do site vêm em CAIXA ALTA, ex. `"MANAUS"`, enquanto `config.json` usa `"Manaus"`).
